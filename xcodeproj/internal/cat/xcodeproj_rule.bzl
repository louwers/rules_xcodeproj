...
def _xcodeproj_impl(ctx):
    ...
    (
        ...
        temp_templates = [
            ctx.file._temp_cat_pbxproject_targets,
            ctx.file._temp_cat_pbxproject_target_attributes,
            ctx.file._temp_cat_pbxtargetdependencies,
        ],
    )
    ...
    for xcode_target in xcode_targets.values():
        ...
        transitive_file_paths.append(
            memory_efficient_depset(xcode_target.inputs.resources),
        )
        transitive_folders.append(
            memory_efficient_depset(xcode_target.inputs.folder_resources),
        )
...
def make_xcodeproj_rule(
    ...
    attrs = {
        ...
        "_temp_cat_pbxproject_target_attributes": attr.label(
            allow_single_file = True,
            default = Label("//xcodeproj/internal/templates:cat.pbxproject_target_attributes"),
        ),
        "_temp_cat_pbxproject_targets": attr.label(
            allow_single_file = True,
            default = Label("//xcodeproj/internal/templates:cat.pbxproject_targets"),
        ),
        "_temp_cat_pbxtargetdependencies": attr.label(
            allow_single_file = True,
            default = Label("//xcodeproj/internal/templates:cat.pbxtargetdependencies"),
        ),
    }
