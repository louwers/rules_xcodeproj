...
def _write_pbxproject_targets(
    ...
    # labels
    args.add_all(_flags.labels, label_counts)
    ...
    actions.run_shell(
        ...
        command = """\
cp -c {pbxproject_targets_template} {pbxproject_targets}
cp -c {pbxproject_target_attributes_template} {pbxproject_target_attributes}
cp -c {pbxtargetdependencies_template} {pbxtargetdependencies}

{temp_consolidation_maps}
""".format(
            pbxproject_targets = pbxproject_targets.path,
            pbxproject_targets_template = temp_templates[0].path,
            pbxproject_target_attributes = pbxproject_target_attributes.path,
            pbxproject_target_attributes_template = temp_templates[1].path,
            pbxtargetdependencies = pbxtargetdependencies.path,
            pbxtargetdependencies_template = temp_templates[2].path,
            temp_consolidation_maps = "\n".join([
                "touch {}".format(consolidation_map.path)
                for consolidation_map in consolidation_maps
            ]),
        ),
...
def _write_consolidation_map_targets(
    ...
    for label in labels:
        for xcode_target in xcode_targets_by_label[label].values():
            ...
            resources_counts.append(len(xcode_target.inputs.resources))
            resources.append(xcode_target.inputs.resources)
            folder_resources_counts.append(
                len(xcode_target.inputs.folder_resources),
            )
    ...
    has_resources = False
    for resources_count in resources_counts:
        if resources_count > 0:
            has_resources = True
            break
    if has_resources:
        # resourcesCounts
        args.add_all(_flags.resources_counts, resources_counts)

        # resources
        args.add_all(_flags.resources, resources, map_each = _paths)

    has_folder_resources = False
    for folder_resources_count in folder_resources_counts:
        if folder_resources_count > 0:
            has_folder_resources = True
            break
    if has_folder_resources:
        # folderResourcesCounts
        args.add_all(_flags.folder_resources_counts, folder_resources_counts)

        # folderResources
        args.add_all(_flags.folder_resources, folder_resources, map_each = _paths)
